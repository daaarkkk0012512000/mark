...
mark = Token:match("(%d+)")
redis:set(mark..":VERSION",1)
redis:set(mark..":SUDO_ID:",SUDO_USER)
redis:set(mark..":DataCenter:",'German')
redis:set(mark..":UserNameBot:",BOT_User)
redis:set(mark..":NameBot:",BOT_NAME)
redis:hset(mark..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3mark_INSTALL","Yes")
info = {}
...
echo -e "\e[38;5;77m       •{ •{ Join the Source Channel : @DEBO_TEAM_3 }• }•\e[0m"
...
echo -e "\e[38;5;77m       •{ SOURCE DEBO source has been installed }•\e[0m"
echo -e "\e[38;5;77m       •{ developer JACK : @V_P_E  }•\e[0m"

echo -e "\e[38;5;77m      DEBO ✾ @V_P_Ee[0m"
...
mark = Token:match("(%d+)")
our_id = tonumber(mark)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(mark..":UserNameBot:")
SUDO_ID = tonumber(redis:get(mark..":SUDO_ID:"))
SUDO_USER = redis:hgetall(mark..'username:'..SUDO_ID).username
version = redis:get(mark..":VERSION")
DataCenter = redis:get(mark..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..mark..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(mark..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(mark..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(mark..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(mark..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(mark..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(mark..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(mark..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(mark..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(mark..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(mark..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(mark..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(mark..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(mark..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(mark..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(mark..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس مارك ⇐ @DEBO_TEAM_3
...
if msg.GroupActive and redis:get(mark..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(mark..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(mark..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(mark..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.mark then 
if msg.text and ScriptFile.imark then
for k, mark in pairs(ScriptFile.mark) do
local SearchText = msg.text:match(mark)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.imark(msg,{msg.text:match(mark)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",mark.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dmark then
if ScriptFile.dmark(msg) == false then
return false
end
...
if Plug.mark then 
if msg.text and Plug.imark then
for k, mark in pairs(Plug.mark) do
local SearchText = msg.text:match(mark)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(mark..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(mark..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.imark(msg,{msg.text:match(mark)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",mark.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source mark")
end 

end
else
print("The File Script.lua Not Runing in The Source mark")
...
function tdcli_update_callback(data)
	...
	redis:srem(mark..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.https://github.com/Sonic11000/jack1eljoker1/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.https://github.com/Sonic11000/jack1eljoker1/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://https://github.com/Sonic11000/jack1eljoker1/main/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	download_file('https://raw.https://github.com/Sonic11000/jack1eljoker1/main/inc/Run.lua','./inc/Run.lua')
	download_file('https://raw.https://github.com/Sonic11000/jack1eljoker1/main/inc/Script.lua','./inc/Script.lua')
	download_file('https://raw.https://github.com/Sonic11000/jack1eljoker1/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.https://github.com/Sonic11000/jack1eljoker1/main/inc/locks.lua','./inc/locks.lua')
	download_file('https://raw.https://github.com/Sonic11000/jack1eljoker1/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.https://github.com/Sonic11000/jack1eljoker1/main/plugins/games.lua','./plugins/games.lua')
	...
	mmd = redis:get(mark..":SUDO_ID:")
redis:hset(mark..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(mark..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(mark..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(mark..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(mark..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(mark..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(mark..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(mark..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(mark..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(mark..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(mark..'group:add'..data.chat_.id_) then
	redis:set(mark..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(mark..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(mark..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(mark..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	mark..'group:name'..k,IDS.Title,
	mark..'num_msg_mark'..k,5,
	mark..'group:add'..k,true,
	mark..'lock_link'..k,true,
	mark..'lock_spam'..k,true,
	mark..'lock_webpage'..k,true,
	mark..'lock_markdown'..k,true,
	mark..'lock_flood'..k,true,
	mark..'lock_bots'..k,true,
	mark..'mute_forward'..k,true,
	mark..'mute_contact'..k,true,
	mark..'mute_document'..k,true,
	mark..'mute_inline'..k,true,
	mark..'lock_username'..k,true,
	mark..'replay'..k,true
	)
	redis:sadd(mark..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(mark..'username:'..ID,'username',user)
	redis:sadd(mark..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(mark..'username:'..ID,'username',user)
	redis:sadd(mark..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(mark..'username:'..ID,'username',user)
	redis:sadd(mark..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(mark..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(mark..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(mark..'username:'..data.user_.id_).username
	...
	redis:hset(mark..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
